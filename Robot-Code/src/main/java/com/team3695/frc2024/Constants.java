// Copyright (c) FIRST and other WPILib contributors.
// Open Source Software; you can modify and/or share it under the terms of
// the WPILib BSD license file in the root directory of this project.

package com.team3695.frc2024;

import com.revrobotics.CANSparkBase.IdleMode;

/**
 * The Constants class provides a convenient place for teams to hold robot-wide
 * numerical or boolean constants. This class should not be used for any other
 * purpose. All constants should be declared globally (i.e. public static). Do
 * not put anything functional in this class.
 *
 * <p>
 * It is advised to statically import this class (or one of its inner classes)
 * wherever the constants are needed, to reduce verbosity.
 */
public final class Constants { // @formatter:off
    public static class SubsystemDriveConstants {
        
        /** Drive subsystem SparkMax device IDs */
        public static final int LEFT_MASTER_ID  = 0,
                                LEFT_SLAVE_ID   = 0,
                                RIGHT_MASTER_ID = 0,
                                RIGHT_SLAVE_ID  = 0;

        /** Drive subsystem SparkMax inverts */
        public static final boolean LEFT_MASTER_INVERT  = false,
                                    LEFT_SLAVE_INVERT   = false,
                                    RIGHT_MASTER_INVERT = false,
                                    RIGHT_SLAVE_INVERT  = false;

        /** Drive subsystem SparkMax current limit */
        public static final int CURRENT_LIMIT = 60;

        /** Drive subsystem SparkMax open loop ramp rate (in seconds) */
        // Might shift to preferences in future commit
        public static final double RAMP_RATE = 0.25D;

        /** Drive subsystem SparkMax idle mode (coast/break) */
        public static final IdleMode IDLE_MODE = IdleMode.kBrake;
    } 
} // @formatter:on
